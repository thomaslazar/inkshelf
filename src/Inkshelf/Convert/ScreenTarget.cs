...
public static class ScreenTarget
{
    ...
    public static RenderTarget FromCookie(string? scr, bool retina = false, bool grayscale = false,
        SpreadMode spread = SpreadMode.Fit)
    {
        if (!string.IsNullOrEmpty(scr))
        {
                ...
                && cw > 0 && ch > 0 && dpr > 0)
            {
                if (retina)
                {
                    ...
                    return new RenderTarget(w, h, dpr, grayscale) { Spread = spread };
                }
                return new RenderTarget(Math.Min(cw, MaxDimension), Math.Min(ch, MaxDimension), 1, grayscale) { Spread = spread };
            }
            ...
            if (p.Length == 2 && int.TryParse(p[0], out var w2) && int.TryParse(p[1], out var h2) && w2 > 0 && h2 > 0)
                return new RenderTarget(Math.Min(w2, MaxDimension), Math.Min(h2, MaxDimension), 1, grayscale) { Spread = spread };
        }
        return new RenderTarget(0, 0, 1, grayscale) { Spread = spread };
    }
}
