...
| Device | Detected resolution | Working settings | Status |
|---|---|---|---|
| Tolino epos 2 | 1442 × 1787 @ dpr 1.875 | retina on, grayscale on, page scale 98, no override | Works |
| Tolino shine | 751 × 909 @ dpr 1.325 | retina on, grayscale on, **override 1021 × 1236 @ ratio 1.325**, page scale has no effect | Works with calibration |

Two further readers have been reported working without special settings; models
to be filled in.

"Detected resolution" is the line the Settings page shows, straight from the
browser probe. It is not necessarily the panel's spec: on the shine the browser
reports 751 × 909 for a 758 × 1024 panel, and its EPUB reader lays pages out in
a space closer to 1021 px wide — hence the override.
...
3. If they are wrong, tick **Override screen resolution**, enter the panel's
   physical pixels and the ratio, save, then use **Regenerate** on the item page.
   The cache key includes these values, so an existing conversion will not
   change on its own.
4. Adjust in ~2% steps, regenerating each time. Both dimensions together: the
   page box keeps the comic's aspect ratio, so trimming only the height shrinks
   the width with it.
